...
{
	//_snwprintf(buffer, size, _T("./APP_MNT_LOG/%s.txt"), logName);
...
inline void Log::InitDebugLog()
{
	...
	m_pDebugLog->SetLogFileCsv(
		//GetLog()->GetLogFileName(buffer, MAX_PATH, _T("AppMntLog")),
		GetLog()->GetLogFileName(buffer, MAX_PATH, _T("log_AppMnt")),
		LOGFILE_PER_DAY, 
		LOG_SIZE, 
		LOG_EVERY_TIME);
}
